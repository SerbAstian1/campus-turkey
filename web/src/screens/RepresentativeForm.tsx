      ...
      {sent ? (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "var(--space-5)", textAlign: "center", padding: "var(--space-6) 0" }}>
          ...
          <p style={{ maxWidth: 460, color: "var(--text-body)" }}>
            Someone reviews every application by hand. Expect a call within one working
            day. Your login is created after that call, so there is nothing to sign in
            with yet.
          </p>
          <div style={{ display: "flex", gap: "var(--space-3)", flexWrap: "wrap", justifyContent: "center" }}>
            <Button variant="secondary" onClick={reset}>Send another application</Button>
            <Button variant="ghost" onClick={() => go("home")}>Back to the website</Button>
          </div>
        </div>
      ) : (
        <form
        ...
        >
          <Input id="r-name" label="Your full name" icon="user" placeholder="Ngozi Eze"
            required autoComplete="name" value={form.fullName} onChange={set("fullName")}
            style={{ gridColumn: "span 2" }} />

          <Input id="r-country" label="Country you are in" icon="globe" placeholder="Nigeria"
            required value={form.country} onChange={set("country")} />

          <Input id="r-territory" label="Territory you want to cover"
            hint="A country, a region, or a set of cities."
            placeholder="South East Nigeria" value={form.territory} onChange={set("territory")} />

          <Input id="r-email" label="Email address" type="email" icon="mail"
            placeholder="you@example.com" required autoComplete="email"
            value={form.email} onChange={set("email")} />

          <Input id="r-phone" label="WhatsApp number" icon="phone"
            hint="Include your country code." autoComplete="tel"
          ...
          <Input id="r-org" label="Organisation (optional)" icon="building-2"
            hint="If you already work through a company."
            placeholder="Eze Education Consultants"
          ...
          <div style={{ gridColumn: "span 2", display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
            ...
            <span style={{ fontFamily: "var(--font-ui)", fontSize: "var(--fs-caption)", color: "var(--text-muted)" }}>
              Where you have placed students, and how you reach them.
            </span>
            <textarea id="r-message" rows={4} value={form.message} onChange={set("message")}
              placeholder="I have worked with families in Enugu and Anambra since 2019."
          ...
          <div style={{ gridColumn: "span 2", display: "flex", flexDirection: "column", gap: "var(--space-5)" }}>
            <Checkbox id="r-consent" label="Campus Turkey may contact me about this application"
              description="We use your details to review and respond. Nothing else."
